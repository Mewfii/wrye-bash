...
class Game(object):
    ...
    def in_master_block(self, modname):
        """Return true for files that load in the masters' block."""
        # FIXME test what is the actual behavior for older games
        return modname.cext == u'.esm' or self.mod_infos[
            modname].isEsm() # check flag
    ...
    def _fix_load_order(self, lord, fix_lo):
        ...
        ol = lord[:]
        lord.sort(key=lambda m: not self.in_master_block(m))
...
class TimestampGame(Game):
    ...
    def __calculate_mtime_order(self, mods=None): # excludes corrupt mods
        ...
        mods.sort(key=lambda x: not self.in_master_block(x))
        return mods
...
class AsteriskGame(Game):
    ...
    max_espms = 254
    max_esls = 4096 # rumor has it that this is too high...
    _ccc_filename = u''
    ...
    # masters' block - esms, esm flagged and esls if present
    def in_master_block(self, modname,
        ...
        return modname.cext in __master_exts or self.mod_infos[modname].isEsm()
