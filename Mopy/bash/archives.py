...
def _extract_command(archivePath, outDirPath, recursive, filelist_to_extract):
    command = [exe7z, u'x', archivePath.s, u'-y', u'-bb1',
               u'-o"%s"' % outDirPath.s, u'-scsUTF-8', u'-sccUTF-8']
    if recursive: command.append(u'-r')
    if filelist_to_extract: command.append(u'@"%s"' % filelist_to_extract)
    return command
