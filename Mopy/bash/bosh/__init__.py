...
class ModInfos(FileInfos):
    ...
    def cached_lo_last_esm(self):
        ...
        for mod in self._lo_wip[1:]:
            if not load_order.in_master_block(mod): return last_esm
            last_esm = mod
        return last_esm
    ...
    def cached_lo_append_if_missing(self, mods):
        ...
        if not new: return
        esms = set(x for x in new if load_order.in_master_block(x))
        if esms:
    ...
    def lo_activate_exact(self, modNames):
        ...
        skipped = load_order.check_active_limit(listToSelect)
        #--Save
        if skipped:
